...
try:
    ...
    surfaces = discover_live_surfaces(cwd="/home/hskim/jarvis", probe=False, which=fake_which_core_only, runner=fake_runner)
    require("availability-only visible status", surfaces["visibleRoutedSession"]["status"], "host-helpers-present-unverified")
    require("availability-only visible readiness issue", validate_bridge_live_readiness(build_bridge_plan(classify_request("fix TypeError in src/hooks/bridge.ts"), cwd="/home/hskim/jarvis"), surfaces), ["visible routed-session helper contracts were not probed"])
    require("availability-only coordinator status", surfaces["coordinatorMcp"]["status"], "not-probed")
    cases.append("availability-only-map")

    surfaces = discover_live_surfaces(cwd="/home/hskim/jarvis", probe=True, which=fake_which_full, runner=fake_runner)
    ...
    holographic_surfaces = discover_live_surfaces(cwd="/home/hskim/jarvis", probe=True, which=fake_which_full, runner=fake_runner_holographic)
    ...
    metadata_only_surfaces = discover_live_surfaces(cwd="/home/hskim/jarvis", probe=True, which=fake_which_no_tmux, runner=fake_runner)
    ...
    invalid_contract_surfaces = discover_live_surfaces(cwd="/home/hskim/jarvis", probe=True, which=fake_which_full, runner=fake_runner_bad_contract)
    ...
    coordinator_plan = build_bridge_plan(coordinator_packet, cwd="/home/hskim/jarvis")
    ...
    mutation_surfaces = discover_live_surfaces(cwd="/home/hskim/jarvis", probe=True, which=fake_which_full, runner=fake_runner)
    ...
    delegate_plan = build_bridge_plan(delegate_packet, cwd="/home/hskim/jarvis")
    ...
    rpc_plan = build_bridge_plan(rpc_packet, cwd="/home/hskim/jarvis")
    ...
    memory_packet = classify_request("remember my API key sk-test-1234567890 for later")
    ...
    sensitive_report = build_live_binding_report(memory_packet, cwd="/home/hskim/jarvis", probe=False)
    ...
    summary = build_summary_memory_record(raw, "User provided sk-test-1234567890; do not persist without approval.")
    ...
    report = build_live_binding_report(packet, cwd="/home/hskim/jarvis", probe=False)
