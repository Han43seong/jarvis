...
686a1d4 feat: add asset candidate review board
```
...
- `slideforge.asset_approval` — visual asset approval gate helpers for the ComfyUI UI / review board / Telegram hybrid flow; `generate-asset-candidates` validates existing image/diagram candidate files and writes `asset-generation-report.json`; `build-asset-review-board` creates HTML/Markdown boards with actual visual candidates, slide titles, recommendations, and approval hints; `approve-assets` records selected candidates into `approved-assets.json`; and `apply-approved-assets` binds only approved `asset_path` values into a new deck JSON with an application report.
...
- `slideforge.cli` — `prepare-sections`, `prepare-deck`, `build-spec`, `generate-asset-briefs`, `compose-html`, `comfyui-handoff`, `smoke-html`, `capture-screenshots`, `export-pptx`, `pptx-delivery-gate`, `export-evidence-pack`, `generate-asset-candidates`, `build-asset-review-board`, `approve-assets`, `apply-approved-assets`, `run-source-local`, `run-design-source-local`, `run-local`, `summarize-run`, and `score-fidelity --markdown-output` artifact commands.
...
# prepare-sections, prepare-deck, build-spec, generate-asset-briefs, compose-html, comfyui-handoff, smoke-html, capture-screenshots, export-pptx, pptx-delivery-gate, export-evidence-pack, approve-assets, generate-asset-candidates, build-asset-review-board, apply-approved-assets, run-source-local, run-design-source-local, run-local, summarize-run, score-fidelity
...
Current implementation supports the full candidate-to-deck source-of-truth step: `generate-asset-candidates` records already-generated ComfyUI/diagram files into `asset-generation-report.json`; `build-asset-review-board` writes HTML/Markdown visual boards; `approve-assets` records ComfyUI UI/review board/messaging selections into `approved-assets.json`; and `apply-approved-assets` writes `deck.approved.json` plus an application report.
```

## Next work

1. Apply the completed visual asset approval gate to the next real customer/RFP deck source: generate candidate files in ComfyUI/diagram renderer, build an asset review board, record approved selections, produce `deck.approved.json`, then run HTML/PPTX/evidence.
