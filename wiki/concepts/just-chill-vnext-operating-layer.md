...
`just-chill` is the renamed and narrowed successor direction for JARVIS vNext. It updates the existing `$HOME/jarvis` project rather than creating a detached replacement repo.
...
- Local raw artifact staging may preserve source evidence and deletion receipts as migration evidence, but canonical promotion requires a host-owned MCP mapping, staging receipts, approval gates, create results, and read-back hash evidence.
- Provider-backed summary/fact-memory plans remain host-owned; just-chill records only supplied receipts and must not call the provider itself. Removal requires prior-add provenance, explicit approval, and a reason.
...
The implementation target is the existing `$HOME/jarvis` repository.
...
6. Live-boundary reports: `just-chill-hermes-live-boundary-v1` distinguishes read visibility from raw artifact storage, models compatible host-owned summary/fact-memory provider tools, preserves host storage authority, and blocks sensitive memory without approval.
...
10. Summary provider receipts: `scripts/just_chill_summary_memory_receipts.py` emits generic host-owned add/remove plans and records local receipts from supplied provider result evidence without provider execution.
...
13. External memory MCP contract: `scripts/just_chill_hermes_memory_mcp.py` exposes host-owned raw artifact, RDF graph, vector sidecar, and status tools with hash, approval, delete, read-back, and retrieval evidence guards; current registration is host-local state.
...
22. Harness MCP wrapper: `scripts/just_chill_harness_mcp.py` exposes the same operations as `just_chill.*` stdio MCP tools. External registration requires explicit operator approval; the wrapper does not execute GJC, write host memory, run SHACL, call coordinator/delegate tools, or search vector stores.
...
Current host registration, provider, tool-version, storage-root, and private topology details are intentionally not published. The public contract requires every live integration to be rediscovered with read-only probes and to remain blocked unless required helpers, explicit mutation consent, approval, and durable evidence are present.
...
- The first implementation is an update inside `$HOME/jarvis`, not a new `$HOME/projects/jarvis-vnext` repo.
